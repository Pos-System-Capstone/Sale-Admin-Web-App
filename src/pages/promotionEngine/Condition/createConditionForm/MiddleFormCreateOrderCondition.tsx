...
export default function MiddleFormCreateOrderCondition({ removeOrderCondition }: Props) {
  ...
  return (
    <>
      ...
      {orderIndex! >= 1 && (
        <Grid item xs={12}>
          <SelectField
            name={`conditionGroups[${conditionGroupIndex}].orderCondition[${
              orderIndex! - 1
            }].nextOperator`}
          ...
          <Grid container item xs={11} spacing={2}>
            <Grid item xs={12}>
              <p>
                ...
                <SelectField
                  name={`conditionGroups[${conditionGroupIndex}].orderCondition[${orderIndex}].quantityOperator`}
                  label=""
                  options={listQuantityOperator}
                />{' '}
                <InputField
                  name={`conditionGroups[${conditionGroupIndex}].orderCondition[${orderIndex}].quantity`}
                  label="Số lượng"
                />
              </p>
            </Grid>
          ...
          <Grid container item xs={11} spacing={2}>
            <Grid item xs={12}>
              <p>
                ...
                <SelectField
                  name={`conditionGroups[${conditionGroupIndex}].orderCondition[${orderIndex}].amountOperator`}
                  label=""
                  options={amountOperator}
                />{' '}
                <InputField
                  name={`conditionGroups[${conditionGroupIndex}].orderCondition[${orderIndex}].amount`}
                  label="Giá trị"
                />
              </p>
            </Grid>
